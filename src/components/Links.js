...
const SidebarLinks = ({ links, level = 0 }) => {
  ...
  const renderLinks = (linkArr, parentIndex = null, currentLevel = 0) => {
    return linkArr.map((link, index) => {
      ...
      const nestedStyle = { marginLeft: `${currentLevel * 22}px` };
  ...
  return (
    <List component="nav" style={{ padding: "30px" }}>
      {renderLinks(links, null, level)}
    </List>
  );
};
